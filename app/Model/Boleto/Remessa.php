...
use App\Model\Boleto\Boleto;
use App\Model\Boleto\Token;

use App\Model\Boleto\Itens\Beneficiario;
use App\Model\Boleto\Itens\GrupoDesconto;
use App\Model\Boleto\Itens\Juros;
use App\Model\Boleto\Itens\Multa;
use App\Model\Boleto\Itens\Pagador;
use App\Model\Boleto\Itens\Moeda;
use App\Model\Boleto\Itens\RecebimentoDivergente;
...
class Remessa {
	...
	public function __construct($config) {
		//setando token
		$token = new Token($config['tipo_ambiente'], $config['client_secret'], $config['client_id']);
	...
	public function addBoleto($data = null) {
		if(!empty($data)) {
			...
			$juros->setPercentualJuros($data['percentual_juros']);
			...
			$multa->setPercentualMulta($data['percentual_multa']);
	...
	public function enviar() {
		...
		function objectToArrayF($object) {
			...
			//CORIGIDO POIS O LARAVEL NÃO ESAVA ENCONTRANDO A FUNÇÃO
			//STEFERSON
	    	return array_map('App\Model\Boleto\objectToArrayF', $array);
		...
		if ($err) {
		  return [false, "2. cURL Error #:" . $err];
		} else {
		  return [$retorno, json_decode($response)];
		}
	}
